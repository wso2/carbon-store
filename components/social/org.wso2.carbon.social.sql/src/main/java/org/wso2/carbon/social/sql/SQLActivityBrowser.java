...
public class SQLActivityBrowser implements ActivityBrowser {
	...
	public static final String SELECT_SQL = "SELECT * FROM "
			+ Constants.SOCIAL_TABLE_NAME + " WHERE " + Constants.CONTEXT_ID_COLUMN
			+ "=?";
	...
	@Override
	public JsonObject getSocialObject(String targetId, String tenantDomain,
			SortOrder order) {
		List<Activity> activities = listActivities(targetId, tenantDomain);
		...
		if (connection != null) {
			...
			try {
				...
				while (resultSet.next()) {
					...
					// TODO get tenantDomain from CarbonContext
					if (tenantDomain.equals(tenant)) {
						Activity activity = new SQLActivity(
								body);
