...
public class MSSQLQueryAdapter extends GenericQueryAdapter implements
		AdapterInterface {
	...
	private static final String COMMENT_SELECT_SQL_DESC = "SELECT * FROM (SELECT SOCIAL_COMMENTS.*, ROW_NUMBER() OVER (ORDER BY ID DESC) AS RN FROM SOCIAL_COMMENTS WHERE payload_context_id=? AND tenant_domain=?) AS X WHERE RN > ? AND RN <= ?";

	private static final String COMMENT_SELECT_SQL_ASC = "SELECT * FROM (SELECT SOCIAL_COMMENTS.*, ROW_NUMBER() OVER (ORDER BY ID ASC) AS RN FROM SOCIAL_COMMENTS WHERE payload_context_id=? AND tenant_domain=?) AS X WHERE RN > ? AND RN <= ?";

	private static final String POPULAR_COMMENTS_SELECT_SQL = "SELECT * FROM (SELECT SOCIAL_COMMENTS.*, ROW_NUMBER() OVER (ORDER BY likes DESC) AS RN FROM SOCIAL_COMMENTS WHERE payload_context_id=? AND tenant_domain=?) AS X WHERE RN > ? AND RN <= ?";

	private static final String POPULAR_ASSETS_SELECT_SQL = "SELECT * FROM (SELECT SOCIAL_RATING_CACHE.*, ROW_NUMBER() OVER (ORDER BY rating_average DESC) AS RN FROM SOCIAL_RATING_CACHE WHERE payload_context_id LIKE ? AND tenant_domain=?) AS X WHERE RN > ? AND RN <= ?";
	...
	public PreparedStatement getPaginatedActivitySetPreparedStatement(
		...
		statement.setInt(4, limit + offset);
