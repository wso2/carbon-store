...
(function (ui, core, asset, app, constants) {
    ...
    var getPageName = function (request, session) {
        ...
        if (options.suffix) {
            ...
            pageDetails.description = getAssetPageDescription(session, options.type, pageDetails.pageName)
            return pageDetails;
        }
        ...
        if (options.suffix) {
            ...
            pageDetails.description = getAppPageDescription(tenantId, pageDetails.pageName)
            return pageDetails;
        }
        return pageDetails;
    };
     ...
     * @return {String}           The title of the provided page
     */
    var getAssetPageDescription = function (session, type, pageName) {
        var pages = asset.getAssetPageEndpoints(session, type);
        var page;
        for (var index = 0; index < pages.length; index++) {
            page = pages[index];
        ...
        return constants.DEFAULT_DESCRIPTION;
    };
    /**
     ...
     * @return {String}          If the page information is found the title is returned,else a default title
     */
    var getAppPageDescription = function (tenantId, pageName) {
        var page = app.getPageEndpoint(tenantId, pageName);
        if (!page) {
            log.warn('Unable to locate meta information for  page: ' + pageName);
            return constants.DEFAULT_DESCRIPTION;
        }
        return page.description || constants.DEFAULT_DESCRIPTION;
    };
    /**
